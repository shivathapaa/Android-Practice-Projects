package com.example.movies.movieList.data.repository

import com.example.movies.movieList.data.local.movie.MovieDatabase
import com.example.movies.movieList.data.mapper.toMovie
import com.example.movies.movieList.data.mapper.toMovieEntity
    ...
    override suspend fun getMovieList(
        ...
        return flow {
            ...
            val movieListFromApi = try {
                ...
                e.printStackTrace()
                emit(Resource.Error(message = "Error loading movies IO"))
                return@flow
            } catch (e: HttpException) {
                e.printStackTrace()
                emit(Resource.Error(message = "Error loading movies Http"))
                return@flow
            } catch (e: Exception) {
                e.printStackTrace()
